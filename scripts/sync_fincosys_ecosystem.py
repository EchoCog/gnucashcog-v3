#!/usr/bin/env python3
"""Sync gnucashcog-v3's cognitive AtomSpace into/out of the shared fincosys
ecosystem (accospace + fincosys + helix +
revstream1/ad-res-j7 case evidence).

This is the operational piece that was missing from the integration
documented in COGNITIVE_ACCOUNTING.md /
libgnucash/engine/gnc-fincosys-bridge.h: that bridge's
gnc_cognitive_export_fincosys_json() / gnc_cognitive_import_fincosys_json()
are C library functions exercised only by
libgnucash/engine/test/test-fincosys-bridge.cpp -- nothing outside the test
...
"GnuCash Ecosystem Sync" section -- to build the combined "gnucash_ecosystem" AtomSpace from fincosys's
master data plus gnucashm's organizations, helix's ecosystem manifest, and
revstream1's case-evidence records, then writes out the shared sync document
as a staged input for gnc_cognitive_import_fincosys_json().

This script does not build or launch gnucashcog-v3 itself. See
...
DEFAULT_OUTPUT = "data/fincosys_sync/gnucashcog_ecosystem_sync.json"
DEFAULT_ATOMSPACE_JSON = "data/fincosys_sync/gnucashcog_ecosystem_atomspace.json"
SOURCE_LABEL = "gnucashcog-v3"
...
def build_config(args):
    ...
    # This repo's own prior export feeds the "gnucashcog-v3" side of the
    # merge in loaders/gnucashcog.py (mapped directly onto HyperNode/HyperEdge
    # instances). It's optional -- there may be no live AtomSpace to export
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--gnucashcog-export",
        help="Path to a prior gnc_cognitive_export_fincosys_json() export "
        "(default: looked up under --fincosys-data-dir)",
    )
    parser.add_argument(
        "--gnucashm-export",
        help="Path to a gnucashm gnc_organizations_to_fincosys_json() export",
    )
    parser.add_argument(
        "--helix-manifest",
    ...
    print(
        "  -> import into the cognitive AtomSpace with "
        "gnc_cognitive_import_fincosys_json() (see "
        "libgnucash/engine/gnc-fincosys-bridge.h)"
    )
