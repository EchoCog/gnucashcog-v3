...
def test_an_order_books_as_balanced_double_entry():
    ...
    assert abs(sum(s["amount"] for s in txn["splits"])) < 1e-6
...
def test_main_writes_a_feed_sync_fincosys_can_read(tmp_path, capsys):
    ...
    accounts, transactions, source = sf.load_feed(str(out))
    assert len(accounts) == 4
    assert len(transactions) == 1
    assert source
...
def test_real_shopify_records_produce_a_clean_sync_plan():
    ...
    assert plan["issues"] == [], plan["issues"][:5]
    assert plan["summary"]["clean"]
    assert plan["summary"]["unbalanced_transaction_count"] == 0
    assert plan["summary"]["unknown_account_ref_count"] == 0
    assert plan["summary"]["duplicate_txid_count"] == 0
    assert plan["summary"]["total_transactions"] == len(transactions)
...
def test_real_quickbooks_records_produce_a_clean_sync_plan():
    ...
    account_recs, txn_recs = ci.to_sync_records(accounts, transactions)
    plan = sf.build_plan(account_recs, txn_recs, "commerce-qbo-fixture")
